...
static Maskbuffer MakeMaskbuffer (const GLint size) {
    ...
    glCache_bindTexture2D (buffer.Texture);
...
// Model display issue on android: eye ball is missing if set GL_DEPTH_COMPONENT, use GL_DEPTH_COMPONENT16 replace! --zhouqi
    /*#if _CSM_COMPONENTS_IOS
      glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, size, size);
    #else*/
    glRenderbufferStorage (GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, size, size); //GL_DEPTH_COMPONENT
//#endif
...
static void ReleaseMaskbuffer (Maskbuffer* buffer) {
    ...
    glCache_deleteTexture (&buffer->Texture);
}
