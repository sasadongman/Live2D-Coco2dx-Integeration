...
static const GLchar NonMaskedFragmentShaderCode[] =
    ...
    "void main()"
    "{"
    "  vec2 uv = vec2(DiffuseUv.x, 1.0 - DiffuseUv.y);"
    "  vec4 fragColor = texture2D(DiffuseTexture, uv) * Color;"
...
static Program MakeProgram (const GLchar* vertexShaderString, const GLchar* fragmentShaderString) {
    ...
    // Use program.
    //glCache_useProgram(program.Handle);
...
static void ReleaseProgram (Program* program) {
    glCache_deleteProgram (program->Handle);
}
...
void ActivateGlProgram (GlProgram program) {
    ...
    glCache_useProgram (Programs[ActiveProgram].Handle);
}
